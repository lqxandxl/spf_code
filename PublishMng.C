...
void PublishMng :: proc_state_pub(TRscMsgHdr * rschdr , TRscMsgBody * rscbody){
    ...
    //cout<<"pubmng begin to proc pubstate msg"<<endl;
    set <string >  :: iterator tmp1;
    rid=from+"_"+rid;
    UniINFO("beigin to proc state pub rid is %s",rid.c_str());
    UniINFO("beigin to proc state pub content is %s",str.c_str());
    ...
    if(tmp1!=stateset->end()){ //find it no proc
       ...
       string topic;
       string content;
       UniINFO("beigin to proc state pub real");
       //通过主题查找要发给谁
        ...
        if (it != root.end()) {//have found
            ...
            UniINFO("beigin to proc state pub topic is %s",topic.c_str());
            UniINFO("beigin to proc state pub content is %s",content.c_str());
            set <string > clientSet;
            clientSet  =  proxy->getSubMng()->getClientForP(topic,from);//add from
            if(!clientSet.empty()){
                ...
                for(;itbegin!=itend;itbegin++){
                   ...
                   //发送队列 msgid 为 from+rid 后面多一个 & topic 使得分解时 可以知道topic 从而知道内容
                    ...
                    //cout<<"need to send pub to "<<to<<endl;
                    UniINFO("need send pub to %s",to.c_str());
                    int res = proxy->send_map_add(to,"state","notify",rid,topic); //rid 包含from local已经处理过了
                    if(res==1){
...
void PublishMng ::clean_stateset(){
    stateset->clear(); //清除 防止拒绝推送消息
}